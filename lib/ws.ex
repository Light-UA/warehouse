defmodule Sample.WS do
  ...
  def init(args) do
    ...
    try do
      ...
      if is_nil(module) do
        ...
        token = case :lists.keyfind("X-Auth-Token", 1, cookies) do
          {_, v} -> v
          _ -> case :lists.keyfind(<<"X-Auth-Token">>, 1, cookies) do
            {_, v} -> v
            _ -> ""
          end
        end
        sid = case :n2o.depickle(token) do
          {{s, _}, _} -> s
          _ -> ""
        end
        ctx = N2O.cx(
  ...
  def handle_in(msg, state) do
    ...
    try do
      case msg do
        {"N2O," <> _ = message, _} -> response(:n2o_proto.stream({:text,message},[],state))
        {"PING", _} -> {:reply, :ok, {:text, "PONG"}, state}
        {message, _} when is_binary(message) -> response(:n2o_proto.stream({:binary,message},[],state))
        other ->
